(function (stylecow) {
	...
	stylecow.Code = function (css, options) {
		...
		if (options.sourceMap) {
			...
			var relativeCssPath = path.resolve(stylecow.cwd(), path.dirname(css.getData('sourceFile')));
			...
			if (inlineSourceMap) {
				if (inlineSourceMap.indexOf('data:application/json;base64,') === 0) {
					...
					previousSourceMap = JSON.parse(fs.readFileSync(path.resolve(relativeCssPath, inlineSourceMap)));
				}
			}
		}
