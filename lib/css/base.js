(function (stylecow) {
	...
	stylecow.Base = Object.create(Array.prototype, {
		...
		clone: {
			value: function () {
				var type = this.parent ? this.parent.type : 'Root';
				var clone = stylecow.parse(this.toString(), new stylecow[type]);
		...
		add: {
			value: function (child, index, after) {
				if (typeof child === 'string') {
					if (index === undefined) {
						return stylecow.parse(child, this);
					}

					return stylecow.parse(child, this, [index, after]);
				}
