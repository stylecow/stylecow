...
function executeTasks (element, tasks) {
	...
	for (var i = tasks._.length - 1; i >= 0; i--) {
		if ((element.parent || element.type === 'Root') && element.is(tasks[i].filter)) {
			tasks[i].fn(element);
		}
	}

	if (element.type in tasks) {
		for (var i = tasks[element.type].length - 1; i >= 0; i--) {
			if ((element.parent || element.type === 'Root') && element.is(tasks[element.type][i].filter)) {
