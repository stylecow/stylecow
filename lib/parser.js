(function (stylecow) {
    ...
    var Parser = function (code, parent, index) {
        this.code = code;
        this.parent = parent;
        this.index = index;
    };
    ...
    Parser.prototype = {
        add: function (item) {
            if (this.index && !this.treeTypes.length) {
                this.current.add(item, this.index[0], this.index[1]);
            } else {
    ...
    stylecow.parse = function (code, parent, index) {
        ...
        var parser = new Parser(code, parent, index);
