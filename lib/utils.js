...
var utils = {
				...
				case '/':
					if (status[0] !== 'comment') {
						if (nextChar === '*') {
							...
							//stringLine not declared in this scope, is this right?
	...
	parseCode: function (code, parent) {
						...
						switch (status[0]) {
							...
							case 'selector':
								...
								if (buffer.indexOf('@import') !== -1) {
									newChild = css.Import.create(buffer);
								} else {
									newChild = css.Declaration.create(buffer);
								}
		...
		if (buffer.trim() && (status[0] === 'root')) {
			child.add(css.Declaration.create(buffer));
		}
